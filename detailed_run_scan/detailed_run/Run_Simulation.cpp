...
void Run_Simulation(std::string filename, int plasmid_flag, int GQ_on_id, double GQ_on_val, int topo_id, double topo_val, int sigma_basal_id, double sigma_basal_val, int world_rank, int restartflag)
{
	double force = 1.0;
	int torque_flag = 0;
	int brute_force_flag = 1;

	sigma_basal = sigma_basal_val;
...
//	}
	...
	std::string outputfolder = "outputfiles/RUN_" + std::to_string(plasmid_flag) + "_" + std::to_string(GQ_on_id) + "_" + std::to_string(topo_id) + "_" + std::to_string(sigma_basal_id);
	std::string inputfolder = "inputfiles/RUN_" + std::to_string(plasmid_flag) + "_" + std::to_string(GQ_on_id) + "_" + std::to_string(topo_id) + "_" + std::to_string(sigma_basal_id);

	Gillespie_Simulation_Multiple_Genes(force, restartflag, brute_force_flag, torque_flag, gene_lengths, TSSes, Gene_Directions, clamp0_flag, clamp1_flag, T, finish_count_limit, promoters, topoisomerase, rna_degrad, barrier_on, barrier_off, barrier, nucl_par, GQ_on_val, fileflag, outputfolder, inputfolder, world_rank, single_pol_II_flag, nucl_file_flag);
...
int main(int argc, char *argv[])
{
	...
	int GQ_on_id = std::stoi(argv[4]);
	double GQ_on_val = std::stod(argv[5]);
	int topo_id = std::stoi(argv[6]);
	double topo_val = std::stod(argv[7]);
	int sigma_basal_id = std::stoi(argv[8]);
	double sigma_basal_val = std::stod(argv[9]);

	generator = std::mt19937(std::time(NULL) + plasmid_flag + GQ_on_id + topo_id + sigma_basal_id + world_rank);

	try
	{
		Run_Simulation(filename, plasmid_flag, GQ_on_id, GQ_on_val, topo_id, topo_val, sigma_basal_id, sigma_basal_val, world_rank, restartflag);
	}
